...
app_log.logger.info("MySQL connection - %s" % app.config["MYSQL_CONNECTION"])

# Spark module is one per application, currently UI Run address is not supported, so we pass UI
# address as dummy value
spark = SparkModule(app.config["SPARK_MASTER_ADDRESS"], app.config["SPARK_UI_ADDRESS"],
...
def jobs_for_status(status):
    template = {}
    # find out Spark status
    template["cluster_status"] = spark.clusterStatus()
    status = str(status).upper()
    statuses = ["ALL", "READY", "RUNNING", "FINISHED", "CLOSED"]
    if status not in statuses:
        status = "ALL"
    template["spark_master"] = app.config["SPARK_MASTER_ADDRESS"]
    template["spark_ui"] = app.config["SPARK_UI_ADDRESS"]
    template["status"] = status
    template["statuses"] = statuses
    template["version"] = app.config["VERSION"]
    return render_template("jobs.html", **template)
