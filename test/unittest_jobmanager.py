...
class JobManagerTestSuite(unittest.TestCase):
    ...
    def test_saveJob(self):
        ...
        self.assertEqual(len(self.storageManager.itemsForKeyspace(
            jobManager.keyspace(job.status))), 1)
        self.assertEqual(len(self.storageManager.itemsForKeyspace(
            jobManager.keyspace(ALL_JOBS_KEY))), 1)
        with self.assertRaises(StandardError):
            jobManager.saveJob({})
        with self.assertRaises(StandardError):
    ...
    def test_changeStatus(self):
        ...
        self.assertEqual(jobManager.jobForUid(job.uid).toDict(), job.toDict())
        self.assertEqual(len(self.storageManager.itemsForKeyspace(
            jobManager.keyspace(job.status), klass=Job)), 1)
        self.assertEqual(len(self.storageManager.itemsForKeyspace(
            jobManager.keyspace(ALL_JOBS_KEY), klass=Job)), 1)
        self.assertEqual(len(self.storageManager.itemsForKeyspace(
            jobManager.keyspace(oldStatus), klass=Job)), 0)
