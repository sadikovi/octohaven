...
class User(object):
    def __init__(self, eid, name, email, created=time.time(), hashkey=None):
        ...
        self._hash = hashkey if hashkey else User.hashkey(self._id, self._created)
    ...
    def create(cls, settings):
        ...
        hashkey = settings["_hash"] if "_hash" in settings else None
        return User(settings["_id"], settings["_name"], settings["_email"], created, hashkey)
...
class Branch(object):
    def __init__(self, name, created=time.time(), edited=time.time()):
        ...
        self._id = Branch.hashid(self._name)
        self._created = created
        self._edited = edited
    ...
    @staticmethod
    def hashid(self, eid):
        return uuid.uuid3(uuid.NAMESPACE_DNS, eid).hex
