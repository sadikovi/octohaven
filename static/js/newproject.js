...
(function() {
  var actions, cancelbtn, changeState, checkvalue, createbtn, dd, dt, first, generate, generatelink, headers, iddl, idinput, idmsg, map, method, msg, namedl, nameinput, newProjectForm, note, randomlink, second, submitForm, url, _ref;
  ...
  checkvalue = function(dl, input, p) {
    ...
    this.loader.sendrequest("get", "/project/new/validate?projectid=" + value, {}, null, (function(_this) {
      return function(ok, res) {
        if (res && ok === 200) {
          res = JSON.parse(res);
          return changeState(dl, input, p, "" + res.status, "" + res.message);
        }
      };
    })(this), (function(_this) {
      return function(ko, res) {
        if (res && (400 <= ko && ko <= 401)) {
          res = JSON.parse(res);
          return changeState(dl, input, p, "" + res.status, "" + res.message);
        } else {
          return changeState(dl, input, p, "error", "Something bad happened on our side");
        }
      };
    })(this));
  ...
  _ref = [
    "post", "/project/new/create", {
      "Content-Type": "application/x-www-form-urlencoded"
    }
  ], method = _ref[0], url = _ref[1], headers = _ref[2];

  submitForm = function() {
    return this.loader.sendrequest(method, url, headers, "projectid=" + idinput.value + "&projectname=" + nameinput.value, function(ok, res) {
      return console.log(ok, res);
    }, function(ko, res) {
      return console.log(ko, res);
    });
  };
